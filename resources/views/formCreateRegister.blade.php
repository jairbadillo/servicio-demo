...
@section('layoutContent')
    ...
    <div class="card mx-auto shadow m-4" style="max-width: 30rem;">
        <div class="card-header bg-transparent font-bold fs-3">Formulario</div>
            <div class="card-body">
                <h3 class="card-title">Agregue un nuevo servicio:</h3>
                <form method="POST" action="{{ route('register.store') }}">
                    @csrf
                    <div class="form-floating form-floating-outline mb-4">
                        <input type="text" class="form-control" id="name" name="name" placeholder="Nombre del servicio..." value="{{ old('name') }}"/>
                    ...
                    <div class="form-floating form-floating-outline mb-4">
                        <input type="date" class="form-control" id="date_expiration" name="date_expiration" placeholder="Fecha de vencimiento" value="{{ old('date_expiration') }}"/>
                    ...
                    <div class="form-floating form-floating-outline mb-4">
                        <input type="number" class="form-control" id="balance" name="balance" placeholder="Saldo..." value="{{ old('balance') }}"/>
                    ...
                    <div class="form-floating form-floating-outline mb-4">
                        <select class="form-select" id="status" name="status">
                            @foreach(config('estados.lista') as $codigo => $nombre)
                                <option value="{{ $codigo }}" {{ old('status') == $codigo ? 'selected' : '' }}>{{ $nombre }}</option>
                            @endforeach
                        </select>
